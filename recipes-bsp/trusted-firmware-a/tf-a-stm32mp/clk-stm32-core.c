...
int clk_stm32_enable_call_ops(struct stm32_clk_priv *priv, uint16_t id)
{
	...
	if ((ops->is_enabled != NULL) && ops->is_enabled(priv, id)) {
		NOTICE("clk_stm32_enable_call_ops: Already enabled with id = %i\n", id);
		return 0;
	}

	if (ops->enable != NULL) {
		NOTICE("clk_stm32_enable_call_ops: Enabling with id = %i\n", id);
		ops->enable(priv, id);
	}
...
static int _clk_stm32_enable_core(struct stm32_clk_priv *priv, int id)
{
	...
	if (priv->gate_refcounts[id] == 0U) {
		...
		if (parent != CLK_IS_ROOT) {
			NOTICE("_clk_stm32_enable_core: calling _clk_stm32_enable_core\n");
			ret = _clk_stm32_enable_core(priv, parent);
			if (ret != 0) {
				NOTICE("_clk_stm32_enable_core: _clk_stm32_enable_core is != 0 (%i)\n", ret);
				return ret;
			}
		}
	...
	NOTICE("_clk_stm32_enable_core: Increase priv->gate_refcounts[%i] from %i to %i\n", id, priv->gate_refcounts[id], priv->gate_refcounts[id] + 1);
...
int _clk_stm32_enable(struct stm32_clk_priv *priv, int id)
{
	...
	NOTICE("_clk_stm32_enable: CLK Unlock: %i\n", ret );
...
bool _clk_stm32_is_enabled(struct stm32_clk_priv *priv, int id)
{
	...
	NOTICE("_clk_stm32_is_enabled: priv->gate_refcounts[%i] == %i\n", id, priv->gate_refcounts[id]);
...
int clk_stm32_init(struct stm32_clk_priv *priv, uintptr_t base)
{
	...
	priv->base = base;
		NOTICE("clk_stm32_init: priv->num = %u\n", priv->num);
	for (i = 0U; i < priv->num; i++) {
		NOTICE("clk_stm32_init: i = %u\n", i);
		const struct stm32_clk_ops *ops = _clk_get_ops(priv, i);

		if (ops->init != NULL) {
			NOTICE("clk_stm32_init: ops->init(priv, %u)\n", i);
			ops->init(priv, i);
		}
	}
